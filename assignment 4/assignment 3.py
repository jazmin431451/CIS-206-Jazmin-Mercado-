...
def display_bmi_table(weight_pounds, height_inches, bmi, classification):
    ...
    """
    assert isinstance(weight_pounds, (int, float)) and isinstance(height_inches, int), \
        "Weight must be given in pounds and height must be given in inches. Received %s" % type(weight_pounds, height_inches)
    assert isinstance(bmi, float) and isinstance(classification, str), \
        "BMI must be a positive number, and classification must be a string. Received % s" % type(bmi, classification)
    
    print("BMI Table:")
    bmi = calculate_bmi(weight_pounds, height_inches)
    classification = classify_bmi(bmi)
    for weight_pounds in range(100, 250, 10):
        print("%.1f" % height_inches, weight_pounds, end="\n")
        print("%.1f" % calculate_bmi(weight_pounds, height_inches))
    for height_inches in range(58, 76, 2):
        print("%.1f" % bmi, end="\n")
        print("%.1f" % classify_bmi(bmi))
...
def main():
   ...
   while True:
    try:
        ...
        display_bmi_table(weight_pounds, height_inches, bmi, classification)
    except:
